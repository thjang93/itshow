...
var notmember = "로그인 해주세요";
var goadmin = "관리자 페이지에서 확인해주세요";

function membercheck(memId, comId, adminId) {
	if(adminId) {
		alert(goadmin)
		return false;
	}
		...
		requestForecast.done(function(data) {
			...
			function doForecast(unit) {
				...
				for (var i = 0; i < length; i++) {
					arr[i] = ("<div class='block'><h5 class='secondary'>" + forecast[i].date + "</h5><h5 class='high'>" + forecast[i][unit].high + "&deg;</h5><h5 class='secondary'>" + forecast[i][unit].low + "&deg;</h5></div>");
				}
