						...
						<div align="center">
							<c:if test="${count gt 0}">
								<c:if test="${startPage gt pageBlock}">
									<a href="memberComList.do">[◀◀]</a>
									<a href="memberComList.do?pageNum=${startPage-pageBlock}">[◀]</a>
								</c:if>
								<c:forEach var="i" begin="${startPage}" end="${endPage}">
									...
									<c:if test="${i != currentPage}">
										<a href="memberComList.do?pageNum=${i}">[${i}]</a>
									</c:if>
								</c:forEach>
								<c:if test="${pageCount gt endPage}">
									<a href="memberComList.do?pageNum=${startPage+pageBlock}">[▶]</a>
									<a href="memberComList.do?pageNum=${pageCount}">[▶▶]</a>
								</c:if>
							</c:if>
						</div>
						<div align="right">
