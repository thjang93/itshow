...
@Controller
public class InfoSeatListHandler implements CommandHandler {
	@Resource
	private InfoDao infoDao;
	
	@RequestMapping("infoSeatList")
	@Override
	public ModelAndView process(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		List<InfoSeatDataBean> infoSeatDto = infoDao.getSeatList(i_t_num);
		//get seat info
		StringBuffer result = new StringBuffer("");
		result.append("{ seats:[");
		for(int i = 0; i < infoSeatDto.size(); i++) {
			result.append("{name : '" + infoSeatDto.get(i).getI_s_level() + "', ");
		...
		return new ModelAndView("info/infoSeatList");
	}
